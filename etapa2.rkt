...
(define (cst-func suffixes)
  (let* ((prefix (longest-common-prefix-of-list suffixes)))
    (cons prefix 
      (map 
        (lambda (x)
          (if (null? x) '() (drop x (length prefix)))
        )
        suffixes
      ))
  )
...
(define (suffixes->st labeling-func suffixes alphabet)
  ;; Dacă lista de sufixe este goală, returnează o listă goală
  (if (collection-empty suffixes)
      '()
      ;; Altfel, procesează sufixele
      (let* ((ch-words (collection-map (lambda (ch) (filter (lambda (suffix) (and (not (null? suffix)) (char=? (car suffix) ch))) suffixes)) alphabet))
             (non-empty-ch-words (collection-filter (lambda (group) (not (null? group))) ch-words)))
        ;; Construiește structura arborelui de sufixe
        (collection-map (lambda (group)
               (let ((label-result (labeling-func group)))
                 ;; Creează o ramură cu eticheta și continuă recursiv pentru restul sufixelor
